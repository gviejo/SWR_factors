...
class LFADS(object):
    ...
    def __init__(self, hps, kind="train", datasets=None):
        ...
        for d, name in enumerate(dataset_names):
            ...
            if datasets and 'alignment_matrix_cxf' in datasets[name].keys():
                ...
                if in_mat_cxf.shape != (data_dim, factors_dim):
                    raise ValueError("""Alignment matrix must have dimensions %d x %d (data_dim x factors_dim), but currently has %d x %d."""% (data_dim, factors_dim, in_mat_cxf.shape[0], in_mat_cxf.shape[1]))
            if datasets and 'alignment_bias_c' in datasets[name].keys():
                ...
                if align_bias_1xc.shape[1] != data_dim:
                    raise ValueError("""Alignment bias must have dimensions %d (data_dim), but currently has %d."""% (data_dim, in_mat_cxf.shape[0]))
        ...
        with tf.variable_scope("glm"):
            ...
            for d, name in enumerate(dataset_names):
                ...
                if hps.output_dist == 'poisson':
                    ...
                    out_fac_lin_mean = init_linear(factors_dim, data_dim, do_bias=True,
                                                mat_init_value=out_mat_fxc,
                                                bias_init_value=out_bias_1xc,
                                                normalized=False,
                    ...
                    out_fac_lin_logvar = init_linear(factors_dim, data_dim, do_bias=True,
        ...
        with tf.variable_scope("z", reuse=False):
            ...
            self.g0s_val = None
            if ic_dim > 0:
                self.prior_zs_g0 = LearnableDiagonalGaussian(batch_size, ic_dim, name="prior_g0",
                                                                mean_init=0.0,
        ...
        gen_cell = gen_cell_class(hps.gen_dim, input_weight_scale=hps.gen_cell_input_weight_scale,
        ...
        opt = tf.train.AdamOptimizer(self.learning_rate, beta1=0.9, beta2=0.999,epsilon=1e-01)
        self.grads = grads
