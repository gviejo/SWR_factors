...
hps = hps_dict_to_obj({
	...
	"lfads_save_dir":               r"..\data\outputs_all", #"model save dir"    
	...
	"factors_dim":                  1,           #"Number of factors from generator"   
	...
	"batch_size":                   200,            #"Batch size to use during training." 
	"learning_rate_init":           0.01,         #"Learning rate initial value"  
	"learning_rate_decay_factor":   0.99,         #"Learning rate decay, decay by this fraction every so often."  
	"learning_rate_stop":           0.0099,        #"The lr is adaptively reduced, stop training at this value."   
	"learning_rate_n_to_compare":   1,            #"Number of previous costs current cost has to be worse than, to lower learning rate."    
...
data = datasets['hd-real']

sess = np.sort([s for s in data if 'Mouse12' in s])[0:2]

data = {s:data[s] for s in sess}

datasets = None



hps.dataset_names = list(data.keys())
hps.dataset_dims = {k:data[k]['data_dim'] for k in data}
hps.num_steps = data[list(data.keys())[0]]['num_steps']
hps.ndatasets = len(data)
